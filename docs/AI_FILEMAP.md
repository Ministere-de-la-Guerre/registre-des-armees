...
- `docs/MOBILE_DIFFERENCES.md` — running catalog of every touch-only fork in the web build (coarse-pointer = phones+tablets; `isPhone()` = phones only, e.g. filter drawer starts collapsed). Keep updated when a mobile-only behavior changes.
