...
## Electron packaging (`web/electron/main.cjs`, `web/package.json`, `web/electron-builder.beta.cjs`)
...
**Release & update channels live in [`docs/RELEASE.md`](RELEASE.md)** — the stable/beta split, the `allowPrerelease=false` design, the build → stage → manual-draft workflow (Windows-only build), and the beta-updater "stuck client" caveat. Read it before cutting a release.
...
Before release: see the checklist and workflow in [`docs/RELEASE.md`](RELEASE.md).
